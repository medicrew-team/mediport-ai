...
[지시사항]
- 답변은 아래 "Answer:"라는 마커 이후에만 작성해야 한다.  
- 최종 출력에는 "Answer:"라는 단어를 포함하지 말고, **답변 내용만 출력**한다.  
- 답변 내용에는 반드시 "{제품명}"과(와) '{성분명}'(들)을 포함해야 한다.
- 답변은 **친근한 상담 느낌**으로 작성해야 하며, 필요할 경우 적절히 느낌표(!)를 사용해 따뜻한 톤을 표현해라.  
- 필요할 경우 상황에 맞는 간단한 이모지를 사용하여 사용자에게 친근함을 주어라. 단, 과도하게 남용하지 말고 적절히 사용한다.  
